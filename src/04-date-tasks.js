...
function isLeapYear(date) {
  return new Date(new Date(date).setMonth(2) - 2000).getDate() === 29;
}
...
function angleBetweenClockHands(date) {
  ...
  const mAngle = (m * 180) / 30;
  const hAngle = ((h + m / 60) * 180) / 6;
  let result = hAngle > mAngle ? hAngle - mAngle : mAngle - hAngle;
